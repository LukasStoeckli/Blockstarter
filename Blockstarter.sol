...
contract Blockstarter {
    ...
    ProjectWithTokens[] public projects;
    
    function createProject(string memory _name, string memory _description, uint _requestedBudget,
    address payable _founder, ProjectUtils.Milestone[] memory _milestones, string memory _tokenSymbol) public {
        projects.push(new ProjectWithTokens(_name, _description, _requestedBudget, _founder, _milestones, _tokenSymbol));
        emit NewProject(projects.length - 1, _name, _requestedBudget);
    }
    
    function createProject(string memory _name, string memory _description, uint _requestedBudget,
    address payable _founder, string[] memory _descriptions, uint[] memory _deadlines, uint[] memory _percentages, string memory _tokenSymbol) public {
        ...
        createProject(_name, _description, _requestedBudget * 1 ether, _founder, milestones, _tokenSymbol);
    }
    
    function defaultTestWithAngels() public {
        ...
        createProject("The default project", "Test project description", 10 ether, msg.sender, milestones, "TEST");
    }
}
